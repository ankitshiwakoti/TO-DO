...
// Add task and store in IndexedDB if offline
...
function displayTask(taskData, taskId) {
    ...
    newTask.querySelector('.complete-btn').addEventListener('click', async () => {
        if (navigator.onLine) {
            ...
            // Update task in IndexedDB while offline
            const tx = idbStore.transaction(['tasks'], 'readwrite');
            const store = tx.objectStore('tasks');
            store.put({ ...taskData, completed: !taskData.completed });
        }
    ...
    newTask.querySelector('.delete-btn').addEventListener('click', async () => {
        if (navigator.onLine) {
            ...
            // Delete task from IndexedDB while offline
            const tx = idbStore.transaction(['tasks'], 'readwrite');
            const store = tx.objectStore('tasks');
            store.delete(taskId);
        }
